...
- 当前状态：P0、P1、P2 已完成首轮实现；P3、P4、P5 仍待推进。
...
验收标准：保留当前 benchmark；增加 `EncodeMulti` 48 kHz benchmark；首轮目标为解码分配降低 15% 以上，编码多消息分配降低 25% 以上，且解码结果不变。
...
验收标准：新增 fuzz 入口；恶意/畸形输入不 panic、不超大分配；CLI 错误路径返回非零退出码和清晰错误。

### P5：功能边界澄清

- `foxhound.go` 的 `DecodeFoxHound` 当前无条件返回 `nil`，调用方难以区分“无解码结果”和“功能未实现”。

验收标准：将 API 调整为返回错误，或在文档中明确未实现状态；增加测试固定当前行为，避免误用。
